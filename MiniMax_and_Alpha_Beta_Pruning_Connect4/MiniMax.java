...
public class MiniMax extends AIModule {
    public void getNextMove(final GameStateModule game) {
        ...
        chosenMove = maxValue(game, 6, initialActivePlayer).move;
        System.out.println("Evaluation: " + eval(game, initialActivePlayer));


    }

    private Pair maxValue(GameStateModule game, int depth, int initialActivePlayer) {
        if (this.terminate) {
        ...
        for (int i = 0; i < game.getWidth(); i++) {
            if (game.canMakeMove(i)) {
                game.makeMove(i);
                Pair pair = new Pair();
                pair = minValue(game, depth-1, initialActivePlayer);
                game.unMakeMove();
                if (pair.value > value) {
                    value = pair.value;
                    finalPair.value = value;
    ...
    private Pair minValue(GameStateModule game, int depth, int initialActivePlayer) {
        if (this.terminate) {
        ...
        for (int i = 0; i < game.getWidth(); i++) {
            if (game.canMakeMove(i)) {
                game.makeMove(i);
                Pair pair = new Pair();
                pair = maxValue(game, depth-1, initialActivePlayer);
                game.unMakeMove();
                if (pair.value < value) {
                    value = pair.value;
                    finalPair.value = value;
