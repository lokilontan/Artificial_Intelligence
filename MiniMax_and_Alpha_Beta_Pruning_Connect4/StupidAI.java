...
public class StupidAI extends AIModule
{
	public void getNextMove(final GameStateModule game)
	{
		...
		//eval(game, game.getActivePlayer());
		System.out.println("At 0:0: " + game.getAt(0,0));
	}
			...
			for (int j = 0; j < game.getHeight(); j++) {
				...
				if (playerAt != 0) {
					if (playerAt == initP) {
						...
						boardEval += potentialWinsOf(game, nonInitPlayer(initP), i, j, 1);
					} else {
	...
	//This function returns the sum of the potential wins and the player's coins placed for this win
		...
		for (int k = 3; k >= 0 ; k--) {
			...
			if (((i-k) >= 0) && (i + (3-k)) < game.getWidth()) {
				for (int ind = i-k; ind < i; ind++) {
					if (game.getAt(ind, j) == nonInitPlayer(player)) {
						break label;
		...
		for (int k = 3; k >= 0 ; k--) {
			...
			if (((j-k) >= 0) && (j + (3-k)) < game.getWidth()) {
				for (int ind = j-k; ind < j; ind++) {
					if (game.getAt(i, ind) == nonInitPlayer(player)) {
						break label;
